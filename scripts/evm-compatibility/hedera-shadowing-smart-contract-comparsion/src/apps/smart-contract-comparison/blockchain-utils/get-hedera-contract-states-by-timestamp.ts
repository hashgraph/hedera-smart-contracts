...
export async function getHederaContractStatesByTimestamp(contractAddress: string, timestamp: string): Promise<StateData[]> {
	const data = await getContractStateByTimestamp(contractAddress, timestamp);
	return data ? data.map(({ slot, value, timestamp, address }) => ({ address, timestamp, slot, value })) : [];
}
