...
abstract contract HtsFungibleToken is IHtsFungibleToken, KeyHelper {
    ...
    // HAPI response codes. See https://github.com/hashgraph/hedera-smart-contracts/blob/main/contracts/system-contracts/HederaResponseCodes.sol
    int32 constant UNKNOWN_CODE = 21; // The responding node has submitted the transaction to the network. Its final status is still unknown.
    int32 constant SUCCESS_CODE = 22; // The transaction was successful.
    ...
    function approve(address spender, uint256 amount) external returns (bool) {
        return IERC20(htsTokenAddress).approve(spender, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
        return IERC20(htsTokenAddress).transferFrom(sender, recipient, amount);
    }
}