...
/**
 * @title HtsFungibleTokenMintBurn
 ...
 */
abstract contract HtsFungibleTokenMintBurn is HtsFungibleToken, Ownable {
    ...
    function _update(address from, address to, uint256 value) internal {
        // Override this function to implement custom logic for updating balances
        if (from == address(0)) {
            (bool success, bytes memory result) = htsSystemContractAddress.call(
                abi.encodeWithSelector(IHederaTokenService.mintToken.selector,
                htsTokenAddress, value, new bytes[](0)));
            (int responseCode, , ) = success ? abi.decode(result, (int32, int64, int64[]))
        ...
        if (to == address(0)) {
            ...
            (int responseCode, ) = success ? abi.decode(result, (int32, int64))
