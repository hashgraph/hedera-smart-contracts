...
import { isApprovalERC721ParamFields } from '@/utils/contract-interactions/erc/constant';
...
const ERC721OperatorApproval = ({ baseContract }: PageProps) => {
  ...
  const transactionResultStorageKey = 'HEDERA.EIP.ERC-721.SET-APPROVAL-RESULTS';
  const approvalStatusStorageKey = 'HEDERA.EIP.ERC-721.APPROVAL-STATUS.READONLY';
    ...
    if (tokenApprovalRes.err) {
      ...
      if (method === 'IS_APPROVAL') {
        ...
        const newApprovalRecords = approvalRecords.map((record) => {
          if (
            record.owner === aprovalRecordObj.owner &&
            record.operator === aprovalRecordObj.operator
          ) {
      ...
      <div className="w-full/ flex flex-col gap-4">
        ...
        <SharedFormInputField
          ...
          handleInputOnChange={(e) =>
            setApproveParams((prev) => ({ ...prev, operator: e.target.value }))
          }
        />
        ...
        <MultiLineMethod
          ...
          handleExecute={() =>
            handleInvokingAPIMethod(
              'IS_APPROVAL',
              isApprovalParams.owner,
              isApprovalParams.operator
            )
          }
          ...
          <Table variant="simple" size={'sm'}>
            <Thead>
              <Tr>
                ...
                <Th color={'#82ACF9'}>Operator</Th>
                <Th color={'#82ACF9'}>IS Approval</Th>
                <Th />
                <Th />
              </Tr>
            </Thead>
              ...
              {approvalRecords.map((record) => {
                ...
                return (
                  <Tr key={`${record.owner}${record.operator}`}>
                    ...
                    <Td
                      onClick={() => copyWalletAddress(record.operator)}
