...
const TransferMultipleTokens = ({ baseContract }: PageProps) => {
  ...
  const handleTransferMultipleTokens = async (API: API_NAMES) => {
    ...
    // extract fungibleParamsReceivers && fungibleParamsAmmounts  array
    const fungibleParamsReceivers = fungibleParamValues.map(
      (prev) => prev.fieldValue.receiverAddress
    );
    const fungibleParamsAmmounts = fungibleParamValues.map((prev) =>
      Number(prev.fieldValue.amount)
    );
    ...
    const sanitizeErr = handleSanitizeHederaFormInputs({
      API: `MULTI_${API}`,
      fungibleReceivers: fungibleParamsReceivers,
      amounts: fungibleParamsAmmounts,
    ...
    let transactionResult: TokenTransferSmartContractResult;
    if (API === 'FUNGIBLE') {
      transactionResult = await transferFungibleTokens(
        baseContract,
        hederaTokenAddress,
        fungibleParamsReceivers,
