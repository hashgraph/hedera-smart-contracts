...
describe('@OZ[TransparentUpgradeableProxy] Transparent Upgradeable Proxy', function () {
  ...
  it('should verify it can change the underlying contract', async function () {
    const factoryBoxV2 = await ethers.getContractFactory(Constants.Contract.BoxV2)
    ...
    const newContractAddressEncoded = '0x000000000000000000000000' + contractBoxV2.address.replace('0x','')
  ...
  it('should verify proxy admin cannot be called by anyone other than owner', async function (){
    ...
    await expect(tx.wait()).to.eventually.be.rejected.and.have.property('code', 'CALL_EXCEPTION')
  })
})
