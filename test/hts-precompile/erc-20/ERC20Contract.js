...
describe("ERC20Contract Test Suite", function () {
  ...
  it("should NOT be able to use transfer", async function () {
    ...
    try {
      const tx = await erc20Contract.connect(signers[0]).transfer(tokenAddress, signers[1].address, amount, Constants.GAS_LIMIT_1_000_000);
      await tx.wait();
    }
    catch (e) {
      expect(e).to.exist;
  ...
  it("should NOT be able to use delegateTransfer", async function () {
    ...
    try {
      const tx = await erc20Contract.connect(signers[0]).delegateTransfer(tokenAddress, signers[1].address, amount, Constants.GAS_LIMIT_1_000_000);
      await tx.wait();
    } catch (e) {
      expect(e).to.exist;
      expect(e.reason).to.eq('transaction failed');
  ...
  it("should NOT be able to use approve", async function () {
    ...
    try {
      const tx = await erc20Contract.connect(signers[0]).approve(tokenAddress, signers[1].address, approvedAmount, Constants.GAS_LIMIT_1_000_000);
      await tx.wait();
    }
    catch (e) {
      expect(e).to.exist;
  ...
  it("should NOT be able to use delegateApprove and allowance", async function () {
    ...
    try {
      const tx = await erc20Contract.connect(signers[0]).delegateApprove(tokenAddress, signers[1].address, approvedAmount, Constants.GAS_LIMIT_1_000_000);
      await tx.wait();
    }
    catch (e) {
      expect(e).to.exist;
  ...
  it("should NOT be able to use delegateTransferFrom", async function () {
    ...
    try {
      const tx = await erc20Contract.connect(signers[1]).delegateTransferFrom(tokenAddress, signers[0].address, signers[1].address, amount, Constants.GAS_LIMIT_1_000_000);
