...
describe("IERC20 Test Suite", function () {
  ...
  it("should be able to get token balance of any account", async function () {
    const contractOwnerBalance = await IERC20.balanceOf(tokenCreateContract.address);
  ...
  it("should be able to approve another account", async function () {
    const signer1AllowanceBefore = await IERC20.allowance(signers[0].address, signers[1].address);
    await IERC20.approve(signers[1].address, AMOUNT, Constants.GAS_LIMIT_800000);
    const signer1AllowanceAfter = await IERC20.allowance(signers[0].address, signers[1].address);
  ...
  it("should be able to execute transferFrom to another account", async function () {
    const tokenCreateBalanceBefore = await IERC20.balanceOf(tokenCreateContract.address);
    const signer0BalanceBefore = await IERC20.balanceOf(signers[0].address);
    const signer1BalanceBefore = await IERC20.balanceOf(signers[1].address);

    await IERC20.approve(signers[1].address, AMOUNT, Constants.GAS_LIMIT_800000);
    const IERC20Signer1 = await IERC20.connect(signers[1]);
    await IERC20Signer1.transferFrom(signers[0].address, tokenCreateContract.address, AMOUNT, Constants.GAS_LIMIT_800000);

    const tokenCreateBalanceAfter = await IERC20.balanceOf(tokenCreateContract.address);
    const signer0BalanceAfter = await IERC20.balanceOf(signers[0].address);
    const signer1BalanceAfter = await IERC20.balanceOf(signers[1].address);
