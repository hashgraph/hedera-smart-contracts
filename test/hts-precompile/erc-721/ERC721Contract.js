...
describe("ERC721Contract Test Suite", function () {
  ...
  before(async function () {
    ...
    await tokenCreateContract.associateTokenPublic(erc721Contract.address, tokenAddress, Constants.GAS_LIMIT_1_000_000);
    await tokenCreateContract.grantTokenKycPublic(tokenAddress, erc721Contract.address);
    await tokenTransferContract.transferNFTPublic(tokenAddress, tokenCreateContract.address, signers[0].address, mintedTokenSerialNumber, Constants.GAS_LIMIT_1_000_000);
  ...
  it("should NOT be able to execute delegateSetApprovalForAll and isApprovedForAll", async function () {
    ...
    await erc721Contract.delegateSetApprovalForAll(tokenAddress, secondWallet.address, true, Constants.GAS_LIMIT_1_000_000);
  ...
  it("should be able to execute delegate transferFrom", async function () {
    ...
    const erc721ContractNFTOwner = await ethers.getContractAt(Constants.Contract.ERC721Contract, erc721Contract.address, firstWallet);
    await erc721ContractNFTOwner.delegateTransferFrom(tokenAddress, firstWallet.address, secondWallet.address, mintedTokenSerialNumber, Constants.GAS_LIMIT_1_000_000);
  ...
  it("should be able to delegate approve", async function () {
    const erc721ContractNFTOwner = await ethers.getContractAt(Constants.Contract.ERC721Contract, erc721Contract.address, secondWallet);
    const beforeApproval = await erc721ContractNFTOwner.getApproved(tokenAddress, mintedTokenSerialNumber, Constants.GAS_LIMIT_1_000_000);
    await erc721ContractNFTOwner.delegateApprove(tokenAddress, firstWallet.address, mintedTokenSerialNumber, Constants.GAS_LIMIT_1_000_000);
    const afterApproval = await erc721ContractNFTOwner.getApproved(tokenAddress, mintedTokenSerialNumber, Constants.GAS_LIMIT_1_000_000);
  ...
  describe("Unsupported operations", async function () {
    ...
    before(async function () {
      ...
      await tokenTransferContract.transferNFTPublic(tokenAddress, tokenCreateContract.address, signers[0].address, serialNumber, Constants.GAS_LIMIT_1_000_000);
    });

    it("should NOT be able to execute approve", async function () {
      const erc721ContractNFTOwner = await ethers.getContractAt(Constants.Contract.ERC721Contract, erc721Contract.address, secondWallet);
      const beforeApproval = await erc721ContractNFTOwner.getApproved(tokenAddress, serialNumber, Constants.GAS_LIMIT_1_000_000);
      await utils.expectToFail(erc721ContractNFTOwner.approve(tokenAddress, firstWallet.address, serialNumber, Constants.GAS_LIMIT_1_000_000), Constants.CALL_EXCEPTION);
      const afterApproval = await erc721ContractNFTOwner.getApproved(tokenAddress, serialNumber, Constants.GAS_LIMIT_1_000_000);
    ...
    it("should NOT be able to execute transferFrom", async function () {
      ...
      const erc721ContractNFTOwner = await ethers.getContractAt(Constants.Contract.ERC721Contract, erc721Contract.address, firstWallet);
      await utils.expectToFail(erc721ContractNFTOwner.transferFrom(tokenAddress, firstWallet.address, secondWallet.address, serialNumber, Constants.GAS_LIMIT_1_000_000), Constants.CALL_EXCEPTION);
    ...
    it("should NOT be able call tokenByIndex", async function () {
      await utils.expectToFail(erc721Contract.tokenByIndex(tokenAddress, 0), Constants.CALL_EXCEPTION);
    });

    it("should NOT be able call tokenOfOwnerByIndex", async function () {
      await utils.expectToFail(erc721Contract.tokenOfOwnerByIndex(tokenAddress, firstWallet.address, 0), Constants.CALL_EXCEPTION);
    });

    it("should NOT be able execute safeTransferFrom", async function () {
      const tx = erc721Contract.safeTransferFrom(tokenAddress, firstWallet.address, secondWallet.address, mintedTokenSerialNumber, Constants.GAS_LIMIT_1_000_000);
    ...
    it("should NOT be able execute safeTransferFromWithData", async function () {
      const tx = erc721Contract.safeTransferFromWithData(tokenAddress, firstWallet.address, secondWallet.address, mintedTokenSerialNumber, '0x01', Constants.GAS_LIMIT_1_000_000);
      await utils.expectToFail(tx, Constants.CALL_EXCEPTION);
    });
  });
});
