...
describe("TokenManagmentContract Test Suite", function () {
    ...
    it('should be able to delete token', async function () {
        ...
        const tokenInfoAfter = (await txAfter.wait()).events.filter(e => e.event === Constants.Events.TokenInfo)[0].args.tokenInfo;
    ...
    it('should be able to freeze and unfreeze token', async function () {
        ...
        const responseCodeFreeze = (await freezeTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        const responseCodeisFrozen = (await isFrozenTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        const isFrozen = (await isFrozenTx.wait()).events.filter(e => e.event === Constants.Events.Frozen)[0].args.frozen;
        ...
        const responseCodeUnfreeze = (await unfreezeTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        const responseCodeisStillFrozen = (await isStillFrozenTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        const isStillFrozen = (await isStillFrozenTx.wait()).events.filter(e => e.event === Constants.Events.Frozen)[0].args.frozen;
    ...
    it('should be able to remove token kyc', async function () {
        ...
        const revokeKycResponseCode = (await revokeKycTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        const isKycResponseCode = (await isKycTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        const isKyc = (await isKycTx.wait()).events.filter(e => e.event === Constants.Events.KycGranted)[0].args.kycGranted;
    ...
    it('should be able to pause and unpause token', async function () {
        ...
        const pauseTokenResponseCode = (await pauseTokenTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        ...
        const uppauseTokenResponseCode = (await unpauseTokenTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
    ...
    it('should be able to update token info', async function () {
        ...
        const tokenInfoBefore = ((await txBeforeInfo.wait()).events.filter(e => e.event === Constants.Events.TokenInfo)[0].args.tokenInfo)[0];
        ...
        const tokenInfoAfter = ((await txAfterInfo.wait()).events.filter(e => e.event === Constants.Events.TokenInfo)[0].args.tokenInfo)[0];
        const responseCodeTokenInfoAfter = (await txAfterInfo.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
    ...
    it('should be able to update token expiry info', async function () {
        ...
        const responseCode = (await getTokenExpiryInfoTxBefore.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        const tokenExpiryInfoBefore = (await getTokenExpiryInfoTxBefore.wait()).events.filter(e => e.event === Constants.Events.TokenExpiryInfo)[0].args.expiryInfo;
        ...
        const updateTokenExpiryInfoTx = (await tokenManagmentContract.updateTokenExpiryInfoPublic(tokenAddress, expiryInfo, Constants.GAS_LIMIT_1_000_000));
        const updateExpiryInfoResponseCode = (await updateTokenExpiryInfoTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;

        // get updated expiryInfo
        const getTokenExpiryInfoTxAfter = (await tokenQueryContract.getTokenExpiryInfoPublic(tokenAddress));
        const getExpiryInfoResponseCode = (await getTokenExpiryInfoTxAfter.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;
        const tokenExpiryInfoAfter = (await getTokenExpiryInfoTxAfter.wait()).events.filter(e => e.event === Constants.Events.TokenExpiryInfo)[0].args.expiryInfo;
    ...
    it('should be able to update token keys', async function () {
        ...
        const originalKey = (await getKeyTx.wait()).events.filter(e => e.event === Constants.Events.TokenKey)[0].args.key;
        const updateKey = [
            false,
        ...
        const updateResponseCode = (await updateTx.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode;

        // Assert updated key
        const tx = await tokenQueryContract.getTokenKeyPublic(tokenAddress, 2);
        const result = await tx.wait();
        const { responseCode } = result.events.filter(e => e.event === Constants.Events.ResponseCode)[0].args;
        const updatedKey = result.events.filter(e => e.event === Constants.Events.TokenKey)[0].args.key;
    ...
    describe('Extended update token info and keys test suite', function () {
        async function getTokenInfo(contract, token) {
            ...
            const tokenInfo = ((await txBeforeInfo.wait()).events.filter(e => e.event === Constants.Events.TokenInfo)[0].args.tokenInfo)[0];
            expect((await txBeforeInfo.wait()).events.filter(e => e.event === Constants.Events.ResponseCode)[0].args.responseCode).to.eq(TX_SUCCESS_CODE);
            return tokenInfo
        };
