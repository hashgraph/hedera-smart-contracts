...
describe("ERC1155 tests", function () {
  ...
  it("should be able to execute balanceOfBatch(address[],uint256[])", async function () {
    const res = await erc1155.balanceOfBatch([signers[0].address, signers[0].address], [tokenId1, tokenId2]);
  ...
  it("should be able to execute isApprovedForAll(address,address)", async function () {
    const res = await erc1155.isApprovedForAll(signers[0].address, signers[1].address);
  ...
  it("should be able to execute safeTransferFrom(address,address,uint256,uint256,bytes)", async function () {
    ...
    await erc1155.safeTransferFrom(signers[0].address, signers[1].address, tokenId1, tradeableAmount, []);
  ...
  it("should be able to execute safeBatchTransferFrom(address,address,uint256[],uint256[],bytes)", async function () {
    const balanceBefore1 = await erc1155.balanceOf(signers[1].address, tokenId1);
    const balanceBefore33 = await erc1155.balanceOf(signers[1].address, tokenId2);
    await erc1155.safeBatchTransferFrom(signers[0].address, signers[1].address, [tokenId1, tokenId2], [tradeableAmount, tradeableAmount], []);
    const balanceAfter1 = await erc1155.balanceOf(signers[1].address, tokenId1);
    const balanceAfter33 = await erc1155.balanceOf(signers[1].address, tokenId2);
