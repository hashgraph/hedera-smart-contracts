...
describe('TokenManagmentContract Test Suite', function () {
  ...
  describe('Update fees', function () {
    ...
    it('should be able to update fixed fee in HTS token', async function () {
      ...
      await utils.associateAndGrantKyc(tokenCreateCustomContract, feeToken, [signers[0].address]);
      ...
      await utils.associateAndGrantKyc(tokenCreateCustomContract, feeToken, [signers[1].address]);

      const grantKycTx = await tokenCreateCustomContract.grantTokenKycPublic(feeToken, tokenCreateCustomContractAddress);
      ...
      const transferFeeTokenToSigner1 = await tokenTransferContract.transferTokensPublic(feeToken, [tokenCreateCustomContractAddress, signers[1].address], [-150, 150], Constants.GAS_LIMIT_1_000_000);
      await transferFeeTokenToSigner1.wait();

      const updatedTokenFeeAmount = tokenFeeAmount + 15;
      const updatedFixedFee = [{amount: updatedTokenFeeAmount, tokenId: feeToken, useHbarsForPayment: false, 
        useCurrentTokenForPayment: false, feeCollector: signers[0].address}];
      const updateFeeTx = await tokenManagmentContract.updateFungibleTokenCustomFeesPublic(tokenWithFees, updatedFixedFee, [])
      const updateFeeResponseCode = (
        await updateFeeTx.wait()
      ).logs.filter((e) => e.fragment.name === Constants.Events.ResponseCode)[0]
        .args.responseCode;

      const balanceBeforeTransferTokenWithFees1 = await utils.getTokenBalance(signers[1].address, tokenWithFees);
      const balanceBeforeTransferTokenWithFees2 = await utils.getTokenBalance(signers[2].address, tokenWithFees);
      const balanceBeforeTransferFeeToken1 = await utils.getTokenBalance(signers[1].address, feeToken);

      const transferBeforeFeeUpdate = await tokenTransferContract.transferTokensPublic(tokenWithFees, [signers[1].address, signers[2].address], [-50, 50], Constants.GAS_LIMIT_1_000_000);
      await transferBeforeFeeUpdate.wait();

      const balanceAfterTransferTokenWithFees1 = await utils.getTokenBalance(signers[1].address, tokenWithFees);
      const balanceAfterTransferTokenWithFees2 = await utils.getTokenBalance(signers[2].address, tokenWithFees);
      const balanceAfterTransferFeeToken1 = await utils.getTokenBalance(signers[1].address, feeToken);

      expect(balanceAfterTransferTokenWithFees1).to.be.equal(balanceBeforeTransferTokenWithFees1 - 50);
      expect(balanceAfterTransferTokenWithFees2).to.be.equal(balanceBeforeTransferTokenWithFees2 + 50);
      expect(balanceAfterTransferFeeToken1).to.be.equal(balanceBeforeTransferFeeToken1 - updatedTokenFeeAmount);


      const tokenInfoTx = await tokenQueryContract.getTokenInfoPublic(tokenWithFees);
    ...
    it('should be able to update fixed fee for HBARs', async function () {
      ...
      const balanceBeforeTransferContract0 = await utils.getHbarBalance(signers[2].address);
      ...
      const balanceAfterTransferContract = await utils.getHbarBalance(signers[2].address);

      expect(parseFloat(balanceAfterTransfer)).to.be.equal(parseFloat(balanceBeforeTransfer0) - parseFloat(tenHbars/utils.tinybarToHbarCoef));
      const updatedFixedFee = [{amount: twentyHbars, tokenId: "0x0000000000000000000000000000000000000000", useHbarsForPayment: true, 
        useCurrentTokenForPayment:false, feeCollector: signers[0].address}]
      
      const updateFeeTx = await tokenManagmentContract.updateFungibleTokenCustomFeesPublic(tokenWithFixedHbarFee, updatedFixedFee, [])
      const updateFeeResponseCode = (
        await updateFeeTx.wait()
      ).logs.filter((e) => e.fragment.name === Constants.Events.ResponseCode)[0]
        .args.responseCode;
      ...
      const balanceAfterUpdateContract = await utils.getHbarBalance(signers[2].address);

      expect(parseFloat(balanceAfterUpdate)).to.be.equal(parseFloat(balanceAfterTransfer) - parseFloat(twentyHbars/utils.tinybarToHbarCoef));
    ...
    it('should be able to update fixed fee in the same token', async function () {
      ...
      const fixedFeeSameTokenUpdated = [{amount: newFeeTokenAmount, tokenId: ethers.ZeroAddress, useHbarsForPayment: false, 
        useCurrentTokenForPayment: true, feeCollector: signers[3].address}];
      const updateFeeTx = await tokenManagmentContract.updateFungibleTokenCustomFeesPublic(tokenWithFixedFeeInSameToken, fixedFeeSameTokenUpdated, []);
      await updateFeeTx.wait();
      ...
      await updateFeeTx.wait()
        ...
        .args.responseCode;
      const tokenInfoTx = await tokenQueryContract.getTokenInfoPublic(tokenWithFixedFeeInSameToken);
    ...
    it('should be able to update multiple fixed fees in HTS token', async function () {
      ...
      const tokenWithFees = await utils.createFungibleTokenWithCustomFeesAndKeys(
        tokenCreateCustomContract,
        signers[0].address,
        [fixedFee, fixedFee2],
        [],
    ...
    it('should be able to update multiple fractional fees in HTS token', async function () {
      ...
      await utils.associateToken(tokenCreateCustomContract, feeToken2, Constants.Contract.TokenCreateCustomContract);

      const fixedFeeAmount = tokenFeeAmount + 50;
      const fractionalFee = {numerator: fractionalFeeNumerator, denominator: fractionalFeeDenominator, minimumAmount: 0, 
        maximumAmount: 0, netOfTransfers: false, feeCollector: signers[0].address};
      const fractionalFee2 = {numerator: fractionalFeeNumerator2, denominator: fractionalFeeDenominator, minimumAmount: 0, 
        maximumAmount: 0, netOfTransfers: false, feeCollector: signers[0].address};
      const fixedFee2 = {amount: fixedFeeAmount, tokenId: feeToken2, useHbarsForPayment: false, 
        useCurrentTokenForPayment: false, feeCollector: signers[0].address};
      const tokenWithFees = await utils.createFungibleTokenWithCustomFeesAndKeys(
        tokenCreateCustomContract,
        signers[0].address,
      ...
      // make a transfer and ensure that the fee is collected
      //apparently first you need to associate and then gran token kyc
      await utils.associateAndGrantKyc(tokenCreateCustomContract, tokenWithFees, [signers[1].address, signers[2].address]);
      const transferTx = await tokenTransferContract.transferTokensPublic(tokenWithFees, [signers[0].address, signers[1].address], [-500, 500]);
      await transferTx.wait();
    ...
    it('should be able to update royalty fee in HBARs for NFT', async function () {
      const fixedFees = [];
      const royaltyFees = [{numerator: 10, denominator: 100, amount: tenHbars, tokenId: ethers.ZeroAddress, useHbarsForPayment: true, feeCollector: signers[2].address}];
      ...
      expect(parseFloat(await utils.getHbarBalance(signers[2].address))).to.equal(beforeNftTransferHbars2 + parseFloat(twentyHbars/utils.tinybarToHbarCoef));
      expect(parseFloat(await utils.getHbarBalance(signers[3].address))).to.equal(beforeNftTransferHbars3 - parseFloat(twentyHbars/utils.tinybarToHbarCoef));
    });
    ...
    it('should be able to update multiple royalty fees in HBARs for NFT', async function () {
      ...
      expect(await utils.getTokenBalance(signers[3].address, nft)).to.equal(1);
      expect(parseFloat(await utils.getHbarBalance(signers[2].address))).to.equal(beforeNftTransferHbars2 + parseFloat((twentyHbars+tenHbars)/utils.tinybarToHbarCoef));
      expect(parseFloat(await utils.getHbarBalance(signers[3].address))).to.equal(beforeNftTransferHbars3 - parseFloat((twentyHbars+tenHbars)/utils.tinybarToHbarCoef));
    });

    it('should be able to update fixed fee in HBARs for NFT', async function () {
      const fixedFees = [{amount: tenHbars, tokenId: ethers.ZeroAddress, useHbarsForPayment: true, 
        useCurrentTokenForPayment: false, feeCollector: signers[2].address}];
      const royaltyFees = [];
      const nft = await utils.createNonFungibleTokenWithCustomRoyaltyFeeAndKeys(
        tokenCreateCustomContract,
      ...
      expect(await utils.getTokenBalance(signers[3].address, nft)).to.equal(1);
      expect(parseFloat(await utils.getHbarBalance(signers[2].address))).to.equal(beforeNftTransferHbars2 + parseFloat(twentyHbars/utils.tinybarToHbarCoef));
      expect(parseFloat(await utils.getHbarBalance(signers[1].address))).to.equal(beforeNftTransferHbars1 - parseFloat(twentyHbars/utils.tinybarToHbarCoef));
    });
    ...
    it('should be able to update fixed HTS fee for NFT', async function () {
      ...
      const fixedFees = [{amount: tokenFeeAmount, tokenId: feeToken, useHbarsForPayment: false, 
        useCurrentTokenForPayment: false, feeCollector: signers[0].address}];
      const royaltyFees = [];
      const nft = await utils.createNonFungibleTokenWithCustomRoyaltyFeeAndKeys(
        tokenCreateCustomContract,
      ...
      const grantKycSigner1FeeTokenReceipt = await grantKycSigner1FeeToken.wait();
      ...
      expect(await utils.getTokenBalance(signers[1].address, feeToken)).to.equal(balanceBeforeSigner1 - (tokenFeeAmount + 13));
      expect(await utils.getTokenBalance(signers[0].address, feeToken)).to.equal(balanceBeforeFeeCollector + (tokenFeeAmount + 13));
      expect(await utils.getTokenBalance(signers[3].address, nft)).to.equal(1);
    })
    ...
    it('should be able to update fixed HTS fee and royalty fee in NFT', async function () {
      ...
      const updateFeeResponseCode = (
        ...
        .args.responseCode;
      const tokenInfoTx = await tokenQueryContract.getNonFungibleTokenInfoPublic(nft, nftTx);
      const tokenInfoResponse = (
        await tokenInfoTx.wait()
    ...
    describe('Update fees negative cases', async function () {
      it('should fail when updating fungible token non-existing fixed fee', async function() {
        ...
        expect(decodeRevertReason).to.equal(CUSTOM_SCHEDULE_ALREADY_HAS_NO_FEES);
      });
      ...
      it('should fail when fee has negative values', async function() {
        ...
        const fixedFee = {amount: tenHbars, tokenId: ethers.ZeroAddress, useHbarsForPayment: false, useCurrentTokenForPayment: true, feeCollector: signers[0].address};
        tokenWithFees = await utils.createFungibleTokenWithCustomFeesAndKeys(
          tokenCreateCustomContract,
          signers[0].address,
          [fixedFee],
      ...
      it('should fail when updating fungible token fees to more than 10', async function() {
        ...
        const fees = [];
        for (let i = 0; i < 11; i++) {
          fees.push({amount: tokenFeeAmount + i, tokenId: ethers.ZeroAddress, useHbarsForPayment: true, useCurrentTokenForPayment: false, feeCollector: signers[0].address});
        }
        const updateFeeTx = await tokenManagmentContract.updateFungibleTokenCustomFeesPublic(tokenWithFees, fees, [])
        try {
          await updateFeeTx.wait();
        } catch (error) {
          transactionHash = error.receipt.hash;
      ...
      it('should fail when updating NFT token fees to more than 10', async function() {
        ...
        const updateFeeTx = await tokenManagmentContract.updateNonFungibleTokenCustomFeesPublic(nft, fees, [])
        try {
          await updateFeeTx.wait();
        } catch (error) {
          transactionHash = error.receipt.hash;
      ...
      it('should fail for updateFungibleTokenCustomFees when token is not associated to fee collector', async function () {
        ...
        const revertReason = await utils.getRevertReasonFromReceipt(transactionHash);
        const decodeRevertReason = utils.decodeErrorMessage(revertReason);
        expect(decodeRevertReason).to.equal(TOKEN_NOT_ASSOCIATED_TO_FEE_COLLECTOR);
      });
