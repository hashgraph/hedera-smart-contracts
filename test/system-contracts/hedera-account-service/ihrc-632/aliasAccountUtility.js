...
describe.only('@HAS IHRC-632 Test Suite', () => {
  let walletA, walletB, walletC, aliasAccountUtility, sdkClient;
  ...
  describe('getEvmAddressAlias', () => {
    it.only('Should execute getEvmAddressAliasPublic and get the corressponded evmAddressAlias', async () => {
      const walletAAccountId = await Utils.getAccountId(
        walletA.address,
        sdkClient
      );

      const walletALongZeroAddress =
        `0x` +
        (await Utils.convertAccountIdToLongZeroAddress(walletAAccountId));

      console.log(`walletALongZeroAddress: ${walletALongZeroAddress}`);
