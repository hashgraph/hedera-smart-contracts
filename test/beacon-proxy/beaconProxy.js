...
describe('@OZBeaconProxy', function () {
  ...
  before(async function () {
    ...
    beacon = await beaconFactory.deploy(contractLogicContractV1.address, owner.address)
  ...
  it('verifies several proxies can be created and used', async function () {
    const beaconProxyFactory2 = await ethers.getContractFactory(beaconProxyContract)
    beaconProxy2 = await beaconProxyFactory2.deploy(beacon.address)
    await beaconProxy2.deployed()
    const eventValueHashed = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("Value(uint256)"))
  ...
  it('verifies underlying contract can be changed', async function () {
    ...
    const eventUpgradedNameHashed = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("Upgraded(address)"))
    const newContractAddressEncoded = '0x000000000000000000000000' + contractLogicContractV2.address.replace('0x','')
