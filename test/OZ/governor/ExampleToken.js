...
describe("ExampleToken", function () {
    ...
    describe("Transactions", function () {
        ...
        it("Should fail if sender doesn’t have enough tokens", async function () {
            ...
            try {
                await token.connect(addr1).transfer(owner.address, 1, Constants.GAS_LIMIT_800000);
            }catch(error){
                const errorData = error.data;
                const errorSignature = ethers.utils.id("ERC20InsufficientBalance(address,uint256,uint256)").substring(0, 10);
