...
describe("Counter Upgradable Contract Test Suite", function () {
  ...
  async function updateProxyContract() {
    ...
    const proxyContract = await upgrades.upgradeProxy(proxyAddress, contract, {
      kind: "uups",
    });
    await proxyContract.deployed();
  ...
  it("should be able to increase and decrease counter on V1", async function () {
    //increment counter
    {
      ...
      expect(counterAfter, 'Asserting counter increment').to.be.greaterThan(counterBefore);
    }
    ...
    //decrement counter
    {
      ...
      const counterAfter = await proxyContractV1.count();
      expect(counterAfter, 'Asserting counter decrement').to.be.lessThanOrEqual(counterBefore);
    }
  });
  ...
  it("should be able to upgrade contract to V2", async function () {
    ...
    expect(addressV1, 'Asserting implementation address is different').to.not.eq(addressV2);
  });
